package com.example.web_content_converter
...
class WebContentConverterPlugin: FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  override fun onAttachedToEngine(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    channel = MethodChannel(flutterPluginBinding.binaryMessenger, "web_content_converter")
            ...
            "contentToImage" -> {
                ...
                webView.webViewClient = object : WebViewClient() {
                    override fun onPageFinished(view: WebView, url: String) {
                        ...
                        val duration = (dheight / 1000 ).toInt() * 200 /// delay 300 ms for every dheight 2000
                        print("\n _duration $duration")
...
//        print("\nthis.scale ${this.scale}")
        val width = (offsetWidth * resources.displayMetrics.density).absoluteValue.toInt()
        val height = (offsetHeight * resources.displayMetrics.density).absoluteValue.toInt()
        print("\nwidth $width")
        print("\nheight $height")
...
fun File.writeBitmap(bitmap: Bitmap, format: Bitmap.CompressFormat, quality: Int) {
    try {
        val file = FileOutputStream(this.path)
        bitmap.compress(format, quality, file)
        file.flush()
        file.close()
    } catch (e: Exception) {
        e.printStackTrace()
