...
import 'package:web_content_converter/revision_info.dart';
...
class ChromeDesktopDirectoryHelper {
  ...
  static FutureOr<String> saveChromeFromAssetToApp({
    ...
    final tagetDirectory =
        io.Directory(p.joinAll([targetPath, zipFileName().replaceAll(".zip", "")]));
    ...
    final zipPath =
        io.Directory(p.joinAll([(await path.getApplicationSupportDirectory()).path, zipFileName()]))
            .path;
...
//https://github.com/maxogden/extract-zip/blob/master/index.js
  ...
  static FutureOr<String> applicationSupportPath() async {
    ...
    return appsDirPath.isEmpty
        ...
        : io.Directory(p.joinAll([supportDir.path, appsDirPath, '.local-chromium', '$revision']))
            .absolute
            .path;
  }
